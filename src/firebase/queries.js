...
export const TagCancelLayaway = async (nBranchCode, layawayNo,layawayForm, showAlert) => {
     let data = [];
    //  console.log('Test: 1. ', layawayNo , layawayForm , showAlert)
    //  alert('Test: 1. ' + layawayNo + ", 2.  " + layawayForm + ", 3. " + showAlert)
     try {
      const docs = await db
        .collection("LayAway")
        .where("layAwayNo", "==", layawayNo)
      ...
      let dCancelled = data[0].cancelled;
      let prodCode = data[0]?.items[0]?.productCode;
      // alert('prodCode: ' + prodCode)
        // console.log('get data: ',data)
      //  console.log('get data1: ',dCancelled,data[0].layawayNo, data.layawayNo)

      //  alert(layawayNo + ", " + dCancelled + ", " + data[0].layawayNo)
      // if (!isValidDate(data.cancelled) || data.cancelled==='' || data.cancelled===undefined) {
      if (isValidDate(dCancelled)) {
          toast.error('Layaway Control No.: ' + layawayNo + ' and Layaway Form No.: ' + layawayForm + ' already tag cancelled');
          if (showAlert){
             alert('Layaway Control No.: ' + layawayNo + ' and Layaway Form No.: ' + layawayForm + ' already tag cancelled');
          } 
          return false;
      }  else {
          toast.success('Layaway Control No.: ' + layawayNo + ' and Layaway Form No.: ' + layawayForm + ' updating...');
          await db.collection('LayAway').doc(layawayNo).update({cancelled: moment(new Date()).format("YYYY-MM-DD")});
          await postCancellLayaway(nBranchCode, layawayNo, moment(new Date()).format("YYYY-MM-DD"), prodCode );
          return true;
      }
